...
public class SlideMenu extends ViewGroup {
	...
	/**
	 * Android拦截机制     http://www.cnblogs.com/sunzn/archive/2013/05/10/3064129.html
	 * 如果这里返回true  则不交给子View进行事件处理,而去执行自己的onTouchEvent()处理
	 * 如果这里返回false  则交给子View去处理
	 */
	@Override
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if(xOffset > yOffset && xOffset > 5) {  //如果水平方向的位移比垂直方向的位移大,则用户可能是想左右滑动     >5是为了防止手抖
				return true;   //拦截事件   交由自己的onTouchEvent()方法处理   不交给子View
			}
